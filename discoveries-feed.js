(function () {
  var CACHE_KEY = "neyron_discoveries_feed_v3";
  ...
  function runFetch() {
    return Promise.all([fetchCrossrefWorks(), fetchCommonsThumbs()])
      .then(function (pair) {
        ...
        writeCache(works.slice(0, 30), thumbs);
  ...
  var CURATED_JSON = "discoveries.json";

  function shuffleCurated(arr) {
    var a = arr.slice();
    var i;
    for (i = a.length - 1; i > 0; i--) {
      var j = Math.floor(Math.random() * (i + 1));
  ...
  function renderCuratedFallback(items) {
    var grid = document.querySelector(GRID_SELECTOR);
    if (!grid || !items || items.length < MIN_CARDS) return false;
    lastWorks = null;
    lastThumbs = null;
    var pool = shuffleCurated(items).filter(function (it) {
      return it && it.link && it.nameEn && it.textEn;
    });
    if (pool.length < MIN_CARDS) return false;
    var pick = pool.slice(0, MIN_CARDS);
    var frag = document.createDocumentFragment();
    var i;
    for (i = 0; i < pick.length; i++) {
      var it = pick[i];
      var nameAz = it.nameAz || it.nameEn;
      var textAz = it.textAz || it.textEn;
      var art = document.createElement("article");
      art.className = "discovery-card";
      art.setAttribute("role", "listitem");
      var a = document.createElement("a");
      ...
      a.target = "_blank";
      a.rel = "noopener noreferrer";
      a.setAttribute("aria-label", it.ariaLabel || it.nameEn || "Link");
      var img = document.createElement("img");
      img.src = it.imgSrc || "";
      img.alt = it.imgAlt || "";
      img.width = 330;
      ...
      var h4 = document.createElement("h4");
      h4.className = "discovery-card__name";
      h4.setAttribute("data-en", it.nameEn || "");
      h4.setAttribute("data-az", nameAz || "");
      var p = document.createElement("p");
      p.className = "discovery-card__text";
      p.setAttribute("data-en", it.textEn || "");
      p.setAttribute("data-az", textAz || "");
      body.appendChild(h4);
      body.appendChild(p);
      art.appendChild(a);
      art.appendChild(body);
    ...
    grid.appendChild(frag);
    var lang = getLang();
    grid.querySelectorAll("[data-en][data-az]").forEach(function (el) {
      el.textContent = lang === "az" ? el.getAttribute("data-az") : el.getAttribute("data-en");
    });
    return true;
  }

  function fetchCuratedFallback() {
    return fetch(CURATED_JSON + "?_=" + Date.now(), { cache: "no-store" })
      .then(function (r) {
        if (!r.ok) throw new Error("curated");
        return r.json();
      })
      .then(function (items) {
        if (!Array.isArray(items) || items.length < MIN_CARDS) return false;
        return renderCuratedFallback(items);
      })
      .catch(function () {
        return false;
      });
  ...
  function init() {
    ...
    if (cached && cached.works.length >= MIN_CARDS) {
      renderCards(shuffleArray(cached.works), cached.thumbs).then(function () {
    ...
    runFetch().then(function (ok) {
      if (ok) {
        maybeRetranslateForAz();
      } else if (!(cached && cached.works.length >= MIN_CARDS)) {
